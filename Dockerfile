...
ARG NEXT_PUBLIC_STORAGE_DRIVER=__NEXT_PUBLIC_STORAGE_DRIVER__
ARG NEXT_PUBLIC_STORAGE_SYNC_URL=__NEXT_PUBLIC_STORAGE_SYNC_URL__
ARG NEXT_PUBLIC_STORAGE_POLL_MS=__NEXT_PUBLIC_STORAGE_POLL_MS__
...
ENV NEXT_PUBLIC_STORAGE_SYNC_URL=$NEXT_PUBLIC_STORAGE_SYNC_URL
...
# 为“同步文件夹”预留可写目录（通常会由宿主机卷挂载覆盖）
RUN mkdir -p /data/sync && chown -R nginx:nginx /data
